...
public class GlazedGroup {
	...
	public DeferredBlock<Block> FLOWER_POT;
	...
	public PatternedGroup GLASS;
	
	/*
	 * Internal build only. Use the long form for compat use.
	 */
	public GlazedGroup(String name, DyeColor c) {
		color = c;
		groupName = name;
		...
		GLASS = new PatternedGroup(name, c);
	}
	
	public GlazedGroup(String name, DyeColor c, DeferredRegister.Blocks regBlock, DeferredRegister.Items regItem, DeferredRegister<DecoratedPotPattern> regPattern, String modid) {
		color = c;
		groupName = name;
		...
		GLASS = new PatternedGroup(modid, c, regBlock, regItem, modid);
	}
	...
	private void build()
	{
		if ( groupName != "glazed") {
			...
			FLOWER_POT = registerBlock(groupName + "_glazed_flower_pot", () -> new GlazedFlowerPot(GlazedGroup.FlowerPotProperties(color)));
	...
	public static Block.Properties GlazedTerracottaProperties(DyeColor c) {
        return BlockBehaviour.Properties.of()
        		.mapColor(c.getMapColor())
        		.requiresCorrectToolForDrops()
        		.strength(1.4F)
        		.pushReaction(PushReaction.PUSH_ONLY);
    }
	
	public static Block.Properties FlowerPotProperties(DyeColor c){
    ...
    public static <T extends Block> DeferredBlock<T> registerBlockOnly(String name, Supplier<T> block) {
        DeferredBlock<T> toReturn = GBLOCK.register(name, block);
        return toReturn;
    }
    ...
    public static <T extends Block> DeferredBlock<T> registerBlockOnly(String name, Supplier<T> block, DeferredRegister.Blocks regBlock) {
        DeferredBlock<T> toReturn = regBlock.register(name, block);
        return toReturn;
    }
